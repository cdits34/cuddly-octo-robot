...
dmForm.onsubmit = async e => {
  ...
  if (members.length > 4) return alert("Group chats are limited to 4 members.");
...
async function updateDMLists() {
  ...
  for (const chatDoc of chatsSnap.docs) {
    ...
    if (!meta || !meta.members || !Array.isArray(meta.memberIds) || !meta.memberIds.includes(user.uid)) continue;
    ...
    if (meta.isGroup) {
      ...
      item.innerHTML = `<img src="${avatar}" alt="pfp"><span class="name">${meta.name || names}</span>${unread > 0 ? `<span class="unread">${unread}</span>` : ""}`;
    } else {
      const other = meta.members.find(m => m.uid !== user.uid) || { displayName: "Unknown", photoURL: "" };
      item.innerHTML = `<img src="${other.photoURL || ""}" alt="pfp"><span class="name">${other.displayName || other.name || other.email || other.uid}</span>${unread > 0 ? `<span class="unread">${unread}</span>` : ""}`;
    }
    item.onclick = async () => {
      await openChatById(chatId);
      backPublicBtn.style.display = "block";
  ...
  for (const uDoc of usersSnap.docs) {
    ...
    const item = document.createElement("div");
    item.className = "dm-item";
    item.innerHTML = `<img src="${otherUser.photoURL || ""}" alt="pfp"><span class="name">${otherUser.displayName || otherUser.email || otherUser.uid}</span>${unread > 0 ? `<span class="unread">${unread}</span>` : ""}`;
    item.onclick = async () => {
